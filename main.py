...
def tasks_from_chunks(json_chunks: Generator[str, None, None]):
  ...
  for chunk in json_chunks:
    ...
    # If there is a remaining fragment that couldn't be parsed in this chunk,
    # yield any tasks from it as well (it might be the last task).
    if remaining:
      try:
        ...
        for task_json in tasks:
          task = Task(**task_json)
          yield task
        remaining = ""  # Reset remaining fragment after parsing the last task.
      except json.JSONDecodeError:
        # The remaining fragment is still incomplete, so we'll keep it for the next chunk.
        pass
